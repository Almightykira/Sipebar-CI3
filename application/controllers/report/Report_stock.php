...
class Report_stock extends CI_Controller
{
    ...
    {
        $data['title']        = 'Laporan';
        $data['subtitle']    = 'Semua data laporan stock akan muncul disini';
    ...
    public function cetak()
    {
        ...
        $pdf->Cell(0, 7, 'Laporan Data Stock ', 0, 1, 'C');
        $pdf->Cell(0, 7,  $data1, 0, 1, 'C');
        $pdf->Cell(10, 7, '', 0, 1);
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(10, 6, 'No', 1, 0, 'C');
        $pdf->Cell(20, 6, 'Date ', 1, 0, 'C');
        $pdf->Cell(37, 6, 'Part Number', 1, 0, 'C');
        $pdf->Cell(34, 6, 'Description', 1, 0, 'C');

        $pdf->Cell(34, 6, 'Category', 1, 0, 'C');
        $pdf->Cell(30, 6, 'Brand', 1, 0, 'C');
        $pdf->Cell(32, 6, 'Move Type', 1, 0, 'C');
        $pdf->Cell(28, 6, 'Price', 1, 0, 'C');
        $pdf->Cell(28, 6, 'Remarks', 1, 0, 'C');
        $pdf->Cell(20, 6, 'Stock', 1, 1, 'C');
        ...
        foreach ($gangguan as $data) {
            ...
            $pdf->Cell(37, 6, $data->part_number, 1, 0);
            $pdf->Cell(34, 6, $data->description, 1, 0);
            ...
            foreach ($this->db->get('tb_brand')->result() as $a) {
                ...
                $pdf->Cell(30, 6, $a->brand, 1, 0);
            }
            $pdf->Cell(32, 6, $data->move_type, 1, 0);
            $pdf->Cell(28, 6, $data->price, 1, 0);
            $pdf->Cell(28, 6, $data->remarks, 1, 0);
